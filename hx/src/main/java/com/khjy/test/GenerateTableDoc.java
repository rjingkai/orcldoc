...
public class GenerateTableDoc {
    ...
    static{
        targetTable.add("SJJH_WA_JQXX");
        targetTable.add("SJJH_WA_AJXX");
        targetTable.add("SJJH_WA_SJJBXX");
        targetTable.add("SJJH_WA_JQXX_INFO");

        targetTable.add("SJJH_WA_AJXX_INFO");
        targetTable.add("T_DEL_FILE");
        targetTable.add("T_FX_DATA");
        targetTable.add("T_FX_DATA_LS");

        targetTable.add("T_GCDJ_ORG");
        targetTable.add("T_GJZ");
        targetTable.add("T_LOG");
        targetTable.add("T_S_BASE_USER");

        targetTable.add("T_S_DEPART");
        targetTable.add("T_S_LOG");
        targetTable.add("T_S_ROLE");
        targetTable.add("T_S_ROLE_ORG");

        targetTable.add("T_S_ROLE_USER");
        targetTable.add("T_S_TIMETASK");


        targetTable.add("T_S_TYPE");
        targetTable.add("T_S_TYPEGROUP");
        targetTable.add("T_S_USER");
        targetTable.add("T_S_USER_ORG");

        targetTable.add("T_SG");
        targetTable.add("T_XZFH");
        targetTable.add("T_ZQB");
        targetTable.add("T_YSFZ");
...
//    private static String url = "*****";//链接url
    ...
    private static String username = "hx"; //用户名.需要设置默认表空间哈
    private static String password = "hx"; //密码
    ...
    public static void addTableDetail(Document document,List columns)throws Exception{
        ...
        Cell cell6 = new Cell("非空");// 单元格
        cell5.setHeader(true);

        Cell cell7 = new Cell("说明");// 单元格
        cell6.setHeader(true);
        //设置表头格式
        table.setWidths(new float[]{8f,30f,15f,8f,10f,9f,20f});
        ...
        for (Iterator iterator = columns.iterator(); iterator.hasNext();) {
            ...
            Cell c6 = new Cell("Y".equals(arr2[4])?"否":"是");
